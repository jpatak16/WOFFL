...
```{r}
a = AllGames %>% filter(season==2022) %>% filter(reg_po=="Regular Season") %>% select(season, week, team, score, perfect_score, opponent, opponent_score)
...
pos_breakdown = AllGames %>% filter(season==CS) %>% filter(reg_po!='BYE') %>% group_by(team) %>% 
  summarise(QB = mean(qb),
            RB = mean(rb1) + mean(rb2),
            WR = mean(wr1) + mean(wr2), 
            TE = mean(te),
            WR_TE= mean(wr_te),
            FLEX = mean(flex),
            DST = mean(dst),
