...
```{r}
library(pacman)
p_load(dplyr, stargazer, data.table, formattable, tidyr, ggplot2, readxl, gganimate, janitor, magrittr, kableExtra, gridExtra, ggalt, forcats)
```
...
afterweek5 = AllGames %>% filter(week<6) %>% group_by(season, team) %>% summarize(overall_wins = sum(overall_wins))
teams4_1 = left_join(teams4_1, afterweek5, by=c('season', 'team'))
...
SetPerfectLineup = function(df){
  ...
  for(r in 1:nrow(df)){
    ...
    #fake names serve the purpose of an ID for the player when filling the roster
    player_list[1,2] = "QB"; player_list[1,3] = df[r, 'qb']; player_list[1,1] = "Eli Manning"
    player_list[2,2] = "RB"; player_list[2,3] = df[r, 'rb1']; player_list[2,1] = "LaDainian Tomlinson"
    player_list[3,2] = "RB"; player_list[3,3] = df[r, 'rb2']; player_list[3,1] = "Brandon Jacobs"
    player_list[4,2] = "WR"; player_list[4,3] = df[r, 'wr1']; player_list[4,1] = "Odell Beckham Jr."
    player_list[5,2] = "WR"; player_list[5,3] = df[r, 'wr2']; player_list[5,1] = "Victor Cruz"
    player_list[6,2] = "TE"; player_list[6,3] = df[r, 'te']; player_list[6,1] = "Jeremy Shockey"
    player_list[7,2] = "DST"; player_list[7,3] = df[r, 'dst']; player_list[7,1] = "Giants"
    player_list[8,2] = df[r,'wr_te_pos']; player_list[8,3] = df[r, 'wr_te']; player_list[8,1] = "Josh Gordon"
    player_list[9,2] = df[r,'flex_pos']; player_list[9,3] = df[r, 'flex']; player_list[9,1] = "Doug Martin"
    original_starters = player_list$Name
    player_list[10,2] = df[r,'b1_pos']; player_list[10,3] = df[r, 'b1']; player_list[10,1] = "Andre Johnson"
    player_list[11,2] = df[r,'b2_pos']; player_list[11,3] = df[r, 'b2']; player_list[11,1] = "Calvin Johnson"
    player_list[12,2] = df[r,'b3_pos']; player_list[12,3] = df[r, 'b3']; player_list[12,1] = "Johnny Manziel"
    player_list[13,2] = df[r,'b4_pos']; player_list[13,3] = df[r, 'b4']; player_list[13,1] = "Michael Strahan"
    player_list[14,2] = df[r,'b5_pos']; player_list[14,3] = df[r, 'b5']; player_list[14,1] = "Tom Coughlin"
    player_list[15,2] = df[r,'b6_pos']; player_list[15,3] = df[r, 'b6']; player_list[15,1] = "Rueben Randle"
    player_list[16,2] = df[r,'b7_pos']; player_list[16,3] = df[r, 'b7']; player_list[16,1] = "David Tyree"
...
PL = AllGames %>% filter(season==2022) %>% 
...
```{r}
a = AllGames %>% filter(season==2022) %>% select(season, week, team, score, perfect_score, opponent, opponent_score)
...
### Schedule Luck Quantified
```{r}
luckAG = AllGames %>% filter(season==CS) %>% filter(week<=CW) %>% 
  select(season, week, team, score, opponent, opponent_score, result, overall_wins, overall_losses) %>%
  mutate(x_wp = overall_wins/9) %>% 
  group_by(team) %>%
  summarise(wins = sum(result), x_wins = sum(x_wp)) %>%
  mutate(luck = wins - x_wins)


scores2022 = AllGames %>% filter(season==CS) %>% filter(week<=CW) %>% select(season, week, team, score)
sims = data.frame(sim_num = NA, team = NA, x_wins = NA); sims = sims[-1,]
for(sim in 1:1000){
  for(t in luckAG$team){
    s = AllGames %>% filter(season==CS) %>% filter(week<=CW) %>% filter(team==t) %>%
